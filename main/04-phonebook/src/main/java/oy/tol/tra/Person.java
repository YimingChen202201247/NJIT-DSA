...
public class Person implements Comparable<Person> {
    ...
    /**
     * Implements the method below to return a hash value.
     * It is calculated from the first and last name of the person.
     *
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < firstName.length(); i++) {
            hash = ((hash << 5) + hash) + firstName.charAt(i); // hash * 33 + c
        }
        for (int i = 0; i < lastName.length(); i++) {
            hash = ((hash << 5) + hash) + lastName.charAt(i); // hash * 33 + c
        }
        return hash;
    }
